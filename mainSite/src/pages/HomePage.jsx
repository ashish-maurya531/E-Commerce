    ...
    try {
      ...
      const featuredProducts = products.slice(0, 4).map(product => ({
        ...
        image: product.images && product.images.length > 0 
          ? `${Src}${product.images[0].replace('/uploads', '/images')}` 
      ...
      // Process products for new arrivals section (next 4 products)
      const newArrivals = products.slice(4, 8).map(product => ({
        id: product.product_id,
        name: product.name,
        price: parseFloat(product.price),
        originalPrice: parseFloat(product.mrp),
            ...
            <>
              <Row gutter={[24, 24]}>
                {visibleCategories.map((category) => (
                  <Col key={category.category_id} xs={24} sm={12} md={8} lg={6}>
                    <Card
